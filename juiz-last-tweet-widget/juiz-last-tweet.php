...
/*
	...
	Plugin URI: https://www.creativejuiz.fr/blog/wordpress/wordpress-plugin-afficher-derniers-tweets-widget
	...
	function juiz_last_tweet_output( $position, $args = array() ) {
		...
		if ( ! function_exists( 'jltw_format_tweettext' ) ) {
			function jltw_format_tweettext( $raw_tweet, $username ) {
				...
				// replace user link
				$i_text = preg_replace('#@([a-zA-z0-9_]+)#i','<a href="https://twitter.com/$1" class="juiz_last_tweet_tweetos" rel="nofollow" target="'.$target4a.'">@$1</a>',$i_text);
				// replace hash tag search link ([a-zA-z0-9_] recently replaced by \S)
				$i_text = preg_replace('#[^&]\#(\S+)#i',' <a href="https://twitter.com/search/?src=hash&amp;q=%23$1" class="juiz_last_tweet_hastag" rel="nofollow" target="'.$target4a.'">#$1</a>',$i_text); // old url was : /search/%23$1
		...
		if ( ! function_exists('jltw_format_tweetsource') ) {
			function jltw_format_tweetsource( $raw_source ) {
				...
				$i_source = preg_replace('#^web$#','<a href="https://twitter.com" rel="nofollow" target="'.$target4a.'">Twitter</a>', $i_source);
		...
		if ( !function_exists('jltw_get_the_user_timeline')) {
			function jltw_get_the_user_timeline($username, $nb_tweets, $show_avatar, $show_action_links, $cache_delay) {
				...
				if (!$current_timer OR $current_timer <= time()) {
					...
					$username = (empty($username)) ? 'geoffreycrofte' : $username;
					...
					if ( is_array( $api_1_1_content ) ) {
						...
						$avatar = $is_object ? $rss_i[0] -> user -> profile_image_url_https : $rss_i[0]['user']['profile_image_url_https'];
						...
						if ($show_avatar && $avatar) { 
							$avatar_attr = array(
								...
								'href'		 => esc_url( 'https://twitter.com/' . $username ),
								'src'		 => esc_url( $avatar ),
								'alt'		 => $author,
								'width'		 => '48',
								'height'	 => '48',
						...
						foreach ( $rss_i as $tweet ) {
							...
							$i_guid		= "https://twitter.com/" . $screename . "/status/" . $id_str;
							...
							if ($show_action_links) {
								...
								$html_action_links ='<span class="juiz_action_links">
									<a title="'.__('Reply', 'juiz-last-tweet-widget').'" href="https://twitter.com/intent/tweet?in_reply_to='.$juiz_tweet_id.'" class="juiz_al_reply" rel="nofollow" target="'.$target4action_links.'">'.__('Reply', 'juiz-last-tweet-widget').'</a> <span class="juiz_ltw_sep">-</span>
									<a title="'.__('Retweet', 'juiz-last-tweet-widget').'" href="https://twitter.com/intent/retweet?tweet_id='.$juiz_tweet_id.'" class="juiz_al_retweet" rel="nofollow" target="'.$target4action_links.'">'.__('Retweet', 'juiz-last-tweet-widget').'</a> <span class="juiz_ltw_sep">-</span>
									<a title="'.__('Favorite', 'juiz-last-tweet-widget').'" href="https://twitter.com/intent/favorite?tweet_id='.$juiz_tweet_id.'" class="juiz_al_fav" rel="nofollow" target="'.$target4action_links.'">'.__('Favorite', 'juiz-last-tweet-widget').'</a> 
								</span>';
							}
			...
			$follow_us = $remove_follow_us ? '' : '<p class="juiz_last_tweet_follow_us">
						...
						<a class="juiz_ltw_username" href="' . esc_url( 'https://twitter.com/' . $the_username ) . '">@' . $the_username . '</a>
